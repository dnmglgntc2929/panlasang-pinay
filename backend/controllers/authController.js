      ...
      req.logIn(user, async (err) => {
        ...
        try {
          let ipAddress = 
            ...
            req.connection?.remoteAddress || // Older Node.js versions
            "Unknown";
  
          // Handle IPv6 loopback (localhost)
          if (ipAddress === "::1" || ipAddress === "127.0.0.1") {
            const axiosResponse = await Axios.get("https://api.ipify.org?format=json");
            ipAddress = axiosResponse.data.ip;
          }
          ...
          return res.status(200).json({ auth: true, token, result: { id: user.id, email: user.email } });
        } catch (error) {
