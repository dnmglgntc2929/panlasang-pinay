...
const LoginLog = createBookshelfModel("login_logs"); 
...
static signUp = async (userData) => {
  ...
  static login = async (userData, ipAddress) => {
    const { email, password } = userData;
    try {
      const User = createBookshelfModel("user_accounts");
      ...
      const token = jwt.sign(
        { id: user.id, email: user.get("email") },
        process.env.JWT_KEY,
        { expiresIn: 259200 } // Token expires in 3 days
      );

      console.log("Inserting login log:", {
        user_id: user.id,
      ...
      // Insert login timestamp into `login_logs`
      await insertInto("login_logs", [
        {
          user_id: user.id,
          email: user.get("email"),
          ip_address: ipAddress,
