...
import { AppBar, Toolbar, Container, Box, Typography, Card, CardContent, Modal, Button, Grid } from "@mui/material";
...
const animalDishes = {
  ...
  const Favorite = () => {
    ...
    useEffect(() => {
      const savedFavorites = JSON.parse(localStorage.getItem("likedDishes")) || {};
      const likedDishes = Object.keys(savedFavorites).filter((key) => savedFavorites[key]);
          ...
          <Grid container spacing={2} justifyContent="center">
            {favorites.length > 0 ? (
              favorites.map((dishName, index) => {
                ...
                const dish = Object.values(animalDishes).flat().find(d => d.name === dishName);
                return dish ? (
